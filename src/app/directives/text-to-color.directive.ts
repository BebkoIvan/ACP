...
ngOnInit(): void {
  ...
  this.renderer.setElementStyle(this.el.nativeElement, 'backgroundColor',this.getColor());
}
...
@HostBinding('class.onHover') private ishovering: boolean;
   ...
   getColor() {
    ...
    switch (title.length) {
      case 2:
        ...
        color=this.colorMap[Math.floor(Math.random()*this.colorMap.length)].color;
        }
        break;
  ...
  @HostListener('mouseover') onHover() {
    if(this.config.hoverReact){
      this.renderer.setElementStyle(this.el.nativeElement, 'backgroundColor',this.LightenColor(this.color,this.config.hoverLighten?30:-30));
